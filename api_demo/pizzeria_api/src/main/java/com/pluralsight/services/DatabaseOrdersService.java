...
public class DatabaseOrdersService implements OrdersService
{
    ...
    @Autowired
    public DatabaseOrdersService(DataSource dataSource, PizzaService pizzaService)
    {
    ...
    @Override
    public ArrayList<Order> searchOrders(String status)
    {
        ...
        String statusSearch = "%" + status + "%";

        try(Connection connection = dataSource.getConnection())
        {
            PreparedStatement statement = connection.prepareStatement(sql);
            statement.setString(1, statusSearch);
